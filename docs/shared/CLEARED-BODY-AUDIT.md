...
**Design verdict (Class 2 — footer states scope, not result).** The footer is pure scope — `"35 rows · N column pairs · p = 1.0"` — it never says nothing was found. This is the card that opened the cleared-body thread. The battery already has the right pattern: Constant-Offset (`"0 blocks detected · 1.1 expected by chance"`) and VFS (`"150 values screened · no values over-represented"`) lead with the result; Duplicate Detection is the outlier that doesn't. **Wrinkle:** unlike those single-signal tests, DupDet runs FOUR sub-signals (collision, identical-row, within-row, block-copies), each with its own observed/expected retained at runtime (`collisionObs=0`, `duplicateRows=0`, `withinRowMatches=0`, `blockCopies=[]`) — there is no single observed/expected pair to show. **Decision: aggregate verdict, mirroring VFS.** Footer → `"35 rows · N column pairs · no duplicates found · p = 1.0"`. Rationale: all four sub-signals are flat zero, so "no duplicates found" is true and complete; it doesn't cram four pairs into a one-liner or arbitrarily privilege one sub-signal (the rejected alternative — leading with block-copies "0 blocks · X expected" — hides that the other three also cleared). The expected-by-chance framing (Constant-Offset's "1.1 expected") earns its place when a count sits NEAR chance; here everything is zero, so the aggregate "no duplicates found" carries the same reassurance without the expected number. Evidence blocks stay as-is (empty on LOW, body collapses to footer). The four-way detail lives in How-this-works. **Implementation:** the `nSpikes===0`-style branch VFS uses (`MiniCard_ValueFrequency.jsx`) is the model — add a "no duplicates found" clause to `footerParts` when all four sub-signal counts are zero on LOW.
...
**Design verdict (Class 1 + Class 3 — RESOLVED S197, 3a-only).** Two problems were in view: (1) the footer prints `primaryP = 0.027`; (2) on LOW the `outlierClause` fell to a **legacy heuristic** (`:44-58`) naming a noisier/quieter column even when nothing flagged (Class-3 "worst-finding on a clean card"). **3b source-check resolved the p question (S197):** the `primaryP` path is **geometry-dependent, not condition-count**. Stratified `minAdjP` (`:200`) fires only for **row-grouped** data (`condCtx.type === 'row-grouped' && count >= 2`, dispatch `:157`); **column-grouped and no-condition** fixtures take the single-run pooled-Bartlett path (`:235`). DS01 is column-grouped → pooled Bartlett `0.02733`. On that path pooled Bartlett **is the test's headline statistic**, and the LOW comes from the effect-size gate (`N≥500 && ratio<3.0`; DS01 N≪500 auto-passes) — so `p = 0.027` is the honest test statistic, not a non-forensic side-cut like Baseline Balance's `ksP`. **Decision: leave the p as `primaryP`.** No cleaner alternative exists on the column-grouped path. **Landed (S197 `38dbec1`): 3a only** — legacy outlier clause suppressed on cleared (LOW/N-A); flagged tiers keep it. LOW footer → `"4 columns · variance ratio 2.7× · Bartlett χ²=9.153 · df=3 · p=0.027"`. **Residual friction → banked:** the footer still leads with "variance ratio 2.7×" + small p with no verdict framing — a reader may wonder why it cleared. This is the same borderline-LOW-verdict-vocabulary item as Benford 1st (block 4) and Blocked Mahalanobis (block 24); fixing it properly is battery-wide verdict-pill work, not card-specific wording. The earlier "max variance ratio · no condition flagged" target was predicated on a cleaner p existing; 3b showed it doesn't, so that wording was dropped. `NoiseSpreadPlot` renders on LOW unchanged.
