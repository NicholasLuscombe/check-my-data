...
**Display.** Tests run on transformed data display a badge (LOG or ANSC) on their test card. The report header shows the VST decision with reasoning. The copy summary includes the full VST reason string for audit, but only when the chosen transform is not raw — a file left on raw carries the reason in the §4 clipboard body alone.

**The reason string named one cause for four (S380, fixed S382).** The fallback branch composed the word `inconclusive` whenever a confidence interval existed, whatever the interval showed. A sheet whose interval sat entirely above 1 and was then overruled by the assay map produced a reason reading, in full: *slope=2.78, CI [2.71, 2.84] above 1 → inconclusive → assay fallback (plate_reader) → raw*. The word described the branch taken, not the measurement made, and the interval it called inconclusive was the most decisive quantity in the file. Because the string reaches `vstProvenance` and is rendered verbatim into the §4 clipboard body noted above, this was a display defect rather than a logging one — and that body is read by an assistant that reasons in the words it is given.

Fixed at S382. The clause now reports which gate turned the promotion away, in four arms tested in order, direction before gate: an interval containing 1 gives `inconclusive`; an interval below 1 gives `additive noise indicated; transform set by assay`; an interval above 1 under any assay other than general gives `CI promotion applies to the general assay only`; and an interval above 1 under the general assay that fails the positivity test gives `CI not applied: data is not on a positive domain`. The arms partition every arrival carrying an interval — measured across all 167 assay-by-fixture combinations, none is uncovered. The residual case cannot reach this branch, and were it ever to, the clause is omitted rather than borrowed from a neighbouring arm.

Two residuals are recorded and not fixed. The third arm requires the general assay, whose fallback is always raw, and the copy summary is gated on a non-raw transform — so that arm can only ever be read in the §4 body. And it names positivity without the negative-fraction figure that the integer and log-fallback branches both report. The restriction the third arm names, and the point null behind it, remain open as P184. Only the wording is settled.
