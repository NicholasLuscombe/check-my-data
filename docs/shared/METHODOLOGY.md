...
## Pooled Dependence (S339–S358)

**One design pattern, found five times.** Dependent units are combined by a null that assumes they are independent. The units differ each time — replicate pairs, condition groups, condition slices, halves of a file — and the failure has the same shape every time: the null is built for a family the data does not contain, so the p it returns is not the quantity the flag reads it as. **This is the review paper's finding.** It is a null-construction defect rather than an arithmetic one, and none of the five is fixed by raising a resample count.
...
4. **Excess Kurtosis, where pooling inverts the sign of the statistic (S341).** Mixing conditions at different residual scales *adds* kurtosis. On DS12b the pooled figure is −0.567, less platykurtic than either condition alone at −0.709 and −0.648, and `kurtDeviation` emerges at **+0.0369** — positive — so directional suppression fires on a signal that is negative in every constituent. The designed detector is silenced by the pooling. The inversion is general and not specific to that fixture (P53); see §2.2.
...
**Instances 4 and 5 sit on one fixture and one run, and they run in opposite directions.** Pooling silences the test built to find the fabrication and manufactures a flag in a different test on the same data. That pair is the sharpest statement of the pattern the project has.
...
**Open on this pattern:** P83 (the Fisher and Šidák arms), P53 (the kurtosis sign inversion), and the cross-regime pooling in Regional Noise itself, which is unfixed and is why DS12b still fails the batch's completeness gate at seed offset 0.
...
**The engine does not currently carry the distinction.** Established by source (S322): an N/A result is `{name, category, flag: "N/A", primaryP: null, description}`, with no reason code, no applicability field, and no enum. The sense lives only in free-text `description`. The single exception is `groupingPending`, a structured marker on the four grouping-held tests — one "not assessed" case, machine-readable, because the confirm card needed it. Preconditions are enforced by per-test early returns with no shared gate; three engine wrappers (`dtSkip`, `condSkip`, `pendingResult`) short-circuit before dispatch. Any partition therefore requires either classifying description strings or introducing a structured field at those guards.

**One known message defect, recorded here because it is a reporting failure and not a bug in the mathematics.** When a per-group-dispatched test finds no group applicable, `aggregatePerGroup` calls the test with an empty matrix to recover a name-and-category prototype, then keeps that call's self-description. Column Goodness-of-Fit, Modality, and Entropy consequently report **"No DATA columns."** on five batch fixtures that have data columns and non-empty groups (DS03, DS04, DS09, DS12a, DS12b) and on the confirmed-thin-grouping path. The state is correct in every case — the tests genuinely could not run — and the accurate reason is already computed and stored one field away, in `details[0].note` ("No group had sufficient data for this test"). Only the displayed description is false. The same string covers two structurally different upstream causes (healthy groups with every column pre-skipped; groups too thin to test) and is indistinguishable from a genuinely empty matrix. These results are **not assessed**, not inapplicable.
