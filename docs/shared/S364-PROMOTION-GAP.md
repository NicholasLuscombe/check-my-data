...
### P120 — **Excess Kurtosis's per-condition statistic is mis-centred against its null across its whole range**

open, **gap closed at S364, cause unmeasured.** `rawP` at the `1/2000` floor on 30 of 478 honest condition-units. **The nominal is 0.05%, not 0.1%** — strict `< 0.001` against a `k/2000` lattice is satisfiable only at the floor — so the multiplier is **126×, not 63×**; read non-strictly it is 80×. Every reading lands between 63 and 126. **It is not only a tail:** median `rawP` 0.256 against 0.5, 67.8% of units below 0.5, every band under 0.1 over-represented. **The promotion gap is answered** — 28 of 30 stop at `kurtosis.js:476-477`, a two-sided p feeding a one-sided family; 1 at BH landing exactly on 0.001; 1 promotes and is capped at MODERATE. The 6.3% and the 1-in-240 measure the condition card and the test flag, and both are right. **The 30 are not 30 independent trials** — they sit on 10 of 20 seeds and repeat down the ratio ladder, so 126× carries no interval. `docs/shared/S364-PROMOTION-GAP.md`, `693f39c` **S364 part B splits it by assay label.** The null/observed ratio is exactly 2 on all 478 units, neither cap active. Per-cell scale fits: on the transformed block `s`/√2 reads **1.050 and 0.935**, so the sample-size mismatch is the whole of the measured inflation and nothing is left over; on the untransformed block **1.383 and 1.717**, a large residual. **One item, two causes.** The single-scale model is rejected in one cell of four — `general` 6rep, 5 floored against 0.9 — but that tail is computed on units, and the 30 units are 15 distinct draws repeating across rungs. **The rejection is untested at draw level and the multiplier still carries no interval.** `docs/shared/S364B-STEP0-CHECKPOINT.md`, `a936557` **S364 Step 0.4 measured the dependence and the rejection strengthened.** The 30 units are 15 blocks, six flooring at all three rungs, against 26.98 blocks expected under a slot-uniform reference — P < 5e-5 over 20,000 permutations. Within-block correlation of the per-condition statistic is **ρ̂ = 0.92 to 0.99**. Dependence makes three occupied blocks *more* surprising, not less: `general` 6rep runs 3 against 0.89 expected at ρ = 0 and 3 against 0.49 at ρ̂, so the tail tightens from 0.059 to 0.013. **Chat's independence estimate reproduced exactly at 0.05876 and at the wrong ρ.** Corrected for four cells it is **0.0507 — a straddle, crossing 0.05 on the modelling assumption alone**, evaluated by quadrature rather than simulation for that reason. **Direction before size:** at ρ = 0 both `plate_reader` cells reject from *below*, which is independence overstating how many blocks a fixed unit count can occupy, and both sit comfortably at ρ̂. `docs/shared/S364B-STEP04-BLOCK-RECOUNT.md`, `442f925` **S364 Step 2 measured the repair and it is large.** Rebuilding the per-condition null at the observed n takes the floored count from **30 units in 15 blocks to 7 units in 3 blocks, all in one cell** — `plate_reader` 6rep. `general` is clean at both replicate counts, median 0.486 and 0.517. **No cell rejects its own fitted scale model**, the decisive cell going from 3 blocks to 0 exactly as predicted, and Step 0.4's straddle resolves with it. Independent confirmation off the body rather than the tail: the re-fitted `s` lands **within 2% of the predicted `s`/√2 in every cell**. **Overall 126× → about 29×, and the whole remainder is the untransformed block at 117×. No cell rejecting is not the same as calibrated.** `docs/shared/S364B-STEP2-MATCHED-N.md`, `5475e7d`
