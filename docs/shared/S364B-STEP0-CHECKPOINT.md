...
### P118 — **six not-transformed tests read raw values against normal-theory analytic nulls**

open. Inter-Replicate Correlation, Mean-Variance Noise Scaling, Within-Row Variance, Cross-Condition Rank Correlation, Value-Frequency Spike, Baseline Balance. Classified at S362 from `engine.js:266-283` with file and line for each. **The configuration that broke P114 is heavy marginals at high correlation, and all six see raw marginals.** Bears on P184 and decision 4 — **the reference was to P113 until S380 closed it, and Mean-Variance Noise Scaling, named in this row's own list, is the test P184 is about** **S364 attaches the first measured residual.** On Excess Kurtosis's per-condition arm the untransformed assay label carries 23 of 30 floored units and a scale residual of 1.383 and 1.717 after the sample-size mismatch is removed, against 1.050 and 0.935 on the transformed block. **The shape is reached by a route P118 does not name** — the routing does apply the transform, but `detectVST` returns `raw` for that label, so `hasVST` is false at `engine.js:300` and it falls through at `:312`. `docs/shared/S364B-STEP0-CHECKPOINT.md`, `a936557` **S364 Step 2 characterises the residual and it is a shift, not only a spread** — mean z +0.97 at six replicates on the raw block, read fit-free by standardising each κ against its own null. **It tracks each condition's own σ monotonically on the raw block** (CondA 0.893 → 0.118 as σ falls, CondB 1.033 → 1.882 as σ rises) **and is flat on the log block**. That is the Gaussian-null signature. **`fitPredictedSigma` is not the main term** — it runs identically on both blocks — though a small log-arm footprint is consistent with it and undemonstrated. `docs/shared/S364B-STEP2-MATCHED-N.md`, `5475e7d` **S364 part C: the mechanism is closed form and confirmed; the magnitude is not.** For replicates independent given the row, a difference's excess kurtosis is `γ/2` — and for log-normal marginals `γ = e^{4s²} + 2e^{3s²} + 3e^{2s²} − 6`, zero after a log transform. **The log block holds exactly** (predicted z 0.03–0.17, measured −0.22–0.06) and the raw block is positive and rising as predicted, but **observed over predicted runs 0.03–0.33 where Chat predicted at or just above 1** — the mixture term `R` is 1.00–1.02 and carries nothing. **The failing premise was the standardisation, not replicate independence** — see P124. **Two exact corrections any future comparison must carry:** row-centring multiplies the residual's excess kurtosis by `c_n = ((n−1)³+1)/((n−1)n²)` and leaves the difference untouched, so a residual-to-difference ratio targets **0.857 at four replicates and 0.714 at six, not 0.5**; and the 2% trim puts the null median at **−0.60 against an untrimmed −0.03**. `docs/shared/S364C-CLOSED-FORM.md`, `9429c55`
