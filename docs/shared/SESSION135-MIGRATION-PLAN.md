...
| **C.6** | DeepLookModal | [DeepLookModal.jsx](src/components/forensics/DeepLookModal.jsx) | Reuses lane labels from §2 surface; but §2 not done yet. Migration involves modal title at `md`. Defer until §2 lands | DS02 (severity 3), DS21 (severity 2) |
| **C.7** | §2 StickySurface + FindingPill + FindingChip | [Section.jsx](src/components/shared/Section.jsx), [StickySurface.jsx](src/components/forensics/StickySurface.jsx), [FindingPill.jsx](src/components/forensics/FindingPill.jsx), [FindingChip.jsx](src/components/forensics/FindingChip.jsx) | Lane label register lands here. `sev.text` retirement at the 3 pill/chip sites. Modal (C.6) re-uses lane label code so order matters — but C.6 can also be folded into this step | DS02, DS04, DS21 |
