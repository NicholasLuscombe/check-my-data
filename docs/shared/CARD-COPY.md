...
- **Noise distribution (Excess Kurtosis)** — the "a too-peaked shape is not forensically meaningful"
  sentence is held pending a methodology audit of the leptokurtic suppression (BANKED). Marked inline.
...
**How this test works.** Takes the difference between each replicate pair and checks the shape of those differences across the dataset. Differences typically follow a bell-shaped curve. Tests for distribution shape, flagging when the spread is too flat — too few values near zero and too few far out (excess kurtosis and Anderson–Darling against a simulated null). A too-peaked shape is not forensically meaningful and does not drive the flag. _[Parked: this directional claim is under methodology audit — see BANKED. Not locked.]_
