...
### 2.10 Row-grouping produces units the tests were not designed for — TRIGGER + CONFIRM CARD BUILT S320–S321, six-commit stack unpromoted, S322 lead
...
**The S322 lead is not a build — it is the card's stance.** S321 closed on a comprehension finding: the card hands a grouping judgment to users (journal editors, integrity officers, editorial-office staff) who may not share the tool's grouping model. That is a methodology question, upstream of any UI polish. The decision (how much the card teaches, whether it leads with a decision or a statement) is blocked on one measurement — **how often the card fires on real data**. The card is an exception surface; the §2.10 census fired 6/12, but the census is built to fire, so it says nothing about real rates. A rare surface justifies heavy inline teaching; a common one (nested-taxonomy ecology data is ordinary — C09's Species/Genus/Family is routine) makes "most users won't know what to do" an adoption problem. The S322 read-only measures the real firing rate first, then Nick sets the stance. Three candidate stances are on the table: teach-at-point-of-need; safe-default (weakened by the C09 N/A case above); state-and-invite-correction (Chat's lean, closest to the screening-instrument voice). The stance likely touches METHODOLOGY §Enforcement — it is a methodology-level call, so consider cross-model validation.

**Three edge-polish copy findings, one round, downstream of the stance** (the stance changes the card's copy, so they are scoped after it):

- "Looks sound" at zero columns → an `attempted`-aware three-state indicator (at zero columns `computeTrigger` returns `pending:false, attempted:false` and the indicator wrongly reads sound).
- "No DATA columns" on too-thin groups → should read "groups too small to test" (the four N/A cards show the aggregator's proto-placeholder; the real reason is too-few-rows-per-group). Sharpest of the three — it is on the most common path, confirming the inferred grouping.
- Size strip flat on balanced designs → render only when sizes vary, or mark equal-height as balanced by design.

The interaction design is recorded in METHODOLOGY §Enforcement (checkbox with live trigger-only recompute; confirm runs the ticked set as-is; informs, does not gate; distinct pending-vs-inapplicable N/A copy; no ranking). The stack unblocks the ecology cluster (C07, C09, C15, C16, C20, C22) on promote.
...
| Row-grouping units (§2.10) | This doc | Single source; **trigger + confirm card BUILT S320–S321, six-commit stack unpromoted (`005026e`), firing-rate + stance the S322 lead.** The applicability failure, in the tool's own grouper. Opened by a performance measurement, not a methodology review. Group census mirrored to `REALWORLD-CORPUS-SPEC.md` §0.3. Contract authored in METHODOLOGY §Enforcement (§Condition Grouping Contract), cross-validated S318. |
