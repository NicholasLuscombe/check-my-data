...
### 2.10 Row-grouping produces units the tests were not designed for — TRIGGER + CONFIRM CARD BUILT S320–S321, stance cross-validated S322, PROMOTED S323 (`07887a5`)

**This is the tool's own applicability failure. It is the third failure mode's strongest exhibit, and it is ours.**

**Status (S367): both parts are built, mechanism-verified, and ON MAIN.** Enforcement is Part 1 (trigger) + Part 2 (confirm card). The **six-commit stack, tip `005026e`**, was built in worktree `beautiful-kalam-efad9d` and **promoted at S323 in merge `07887a5`** — verified by `git merge-base --is-ancestor 005026e main`. **Nothing recorded the promote because the stack rode into main beneath a promote named for something else:** `965749e` (coverage vocabulary) was committed onto the same branch above the stack, and the merge message names only the top commit. **A held stack and a working branch cannot be the same branch.** The S321 status this replaces — "nothing is promoted … the stack does NOT promote until the card's UI settles and its stance is decided" — was the intent at the time; the stance was cross-validated at S322 and the promote followed at S323. Commit chain:
...
| Row-grouping units (§2.10) | This doc | Single source; **trigger + confirm card BUILT S320–S321, stance cross-validated S322, PROMOTED S323 (`07887a5`), verified S367; one display reconciliation outstanding.** The applicability failure, in the tool's own grouper. Opened by a performance measurement, not a methodology review. Group census mirrored to `REALWORLD-CORPUS-SPEC.md` §0.3 (note the S322 C14 correction). Contract and stance authored in METHODOLOGY §Condition Grouping Contract, cross-validated twice (S318 factor-vs-stratum, S322 stance). |
