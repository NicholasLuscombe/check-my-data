...
- **S300 — the arrangement redirect (route to §2.4) is falsified; the fix redirects again, to a multiplicity-distribution statistic (a lead, not yet a locked design).** S299 sent the fix to §2.4 (arrangement detection). S300 tested that at source and closed it. A read established the engine's block-copy Pass 1 reads recurring *row-sequences*, not single-column *runs*; a tracked measurement (`SESSION300-BLOCKCOPY-MEASURE.md`) then showed Pass 1 fires identically on contiguous and periodic arrangements at identical marginals (block p 6.25e-4 both, height h=10), silent only on aperiodic scatter — so its null cannot separate a copy-paste from legitimate periodic structure, and is not re-scopable for §2.4. Decisively, CORPUS-03's real SL defect is **scattered, not contiguous** (`REALWORLD-CORPUS-SPEC.md` CORPUS-03 documented-defect line — each value recurs four times, scrambled across a fish's four rows by an ID-misalignment join, not laid down in a block), which no run/arrangement statistic detects. So the arrangement axis fails CORPUS-03 exactly as the count-null did, for the mirror reason: the count-null read the marginal (no positional signal); the run test reads position (but the defect has none).
    ...
    - **This is a lead, not a locked design — it rests on two unverified assumptions**, both distributional questions to be **measured on tracked data before the statistic is designed**, not reasoned (per S237): (i) that the coincidental-repeat multiplicity distribution for continuous data at CORPUS-03's precision/spread is dispersed enough that a spike-at-k separates from clustered-clean — the same failure mode that closed the count-null (clustered-clean must not mimic the defect); (ii) that "every value ×4" is the true raw shape, not a spec idealisation (the raw column is gitignored; some fish may have 3 or 5 observations, smearing the spike). If clean data also concentrates at low multiplicities, the route is closed and §2.6 becomes an honest disclosed single-column miss in the paper's §5.
    - **Next move (specifiable now).** One measurement: the per-value multiplicity distribution on clean continuous fixtures (09-proteomics, 11-rnaseq, 12a) versus an every-value-×k fixture (`fix-A`'s multiset already has this shape — five values ×24), testing whether the spike separates from clean dispersion. If it separates, design the multiplicity statistic; if not, close the route and disclose.

**Remaining §2.6 gates:**
- **The fix itself — redirected (S299) from count-null repair to arrangement routing; blocked on the §2.4 design pass.** S299 retired the last constrained count-null candidate and established the single-column collision null is under-determined from every precision-derived direction (see the S299 entry). The fix is no longer "replace the continuous collision null with a better baseline" — it is "route structured continuous recurrence to an arrangement/sequence test (the §2.4 column-localised sequential duplication detector) and leave the exact-collision null integer-only." The null path is read and isolated (S298: `duplicateDetection.js:55/:147/:179-184`, one call site), the per-column stratified combine is confirmed clean, and per-column grid step is locked — but these describe the count-null site the redirect steps away from, retained only for the integer-only branch. The immediate unblocking move is the **§2.4 design pass** (§2.4's statistic is still unspecified) plus a **read** confirming what the engine's existing block-copy sub-test keys on and why it reaches SL-alone — that read decides whether the fix is "route continuous recurrence to the existing sub-test" (if it already reads single-column arrangement) or "build §2.4's single-column run test" (if the HIGH depends on whole-row identity that only survives when SL is isolated). Fixture `23`'s DupDet LOW→HIGH assertion is unaffected by the redirect — it flips whenever the arrangement test lands, not the count null. **(S300 supersedes the §2.4 target: the arrangement route is closed — CORPUS-03's defect is scattered, not contiguous, so no run/arrangement statistic reaches it. The fix now targets the per-value multiplicity distribution; see the S300 entry above. Still blocked — on the S300 multiplicity-separation measurement, not the §2.4 design pass. Note also: fixture 23 carries *scattered* recurrence, so it will not flip under a contiguous-run test regardless; whether it flips under a multiplicity statistic depends on that statistic's design.)**
- **The block-copy-reach read + §2.4 design pass (the redirected critical path).** A read (READ-ONLY) of what the engine's block-copy sub-test keys on and why it fires HIGH on SL-alone, which forks the fix between "route to existing sub-test" and "build §2.4's test." Then §2.4's statistic design (currently unspecified) if the build path is taken. This replaces the retired collision-null design pass — S299 proved the count-null route closed. The §2.4 entry is the home for the detector's design; §2.6 consumes it for the continuous-recurrence fix. **(S300: this read is done — `SESSION300-BLOCKCOPY-REACH-READ.md` and `SESSION300-BLOCKCOPY-MEASURE.md`. The fork it framed is moot: the block-copy null reads recurring row-sequences not single-column runs, fires on periodic-clean data, and is not re-scopable; and §2.4's arrangement axis does not fit CORPUS-03's scattered defect. Replaced by the S300 multiplicity-separation measurement as the next unblocking move — see the S300 entry above.)**
...
- **The owed source-comment correction** (`duplicateDetection.js:135`) — landed S297 with the fixture merge; the empirical-HHI circularity claim now carries the known-limitation note (does not claim any fix has landed). Separately, S298 found the comment at `:140-146` overstates the four-channel coupling — a comment fix to fold into the eventual engine change, not its own task.
