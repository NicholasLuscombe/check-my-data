...
**S295 progress — source-derivation read done, fix-scoping advanced.** The structure-first read the Operating model calls for ran this session (three artifacts in `docs/shared/`: the per-test walk `SESSION295-AUDIT-SUMMARY.md`; the build stop-report `SESSION295-IMPL-SUMMARY.md`; the axis-2 null-constraints read `SESSION295-AXIS2-NULL-CONSTRAINTS.md`). What it settled:
...
- **Axis 2 (continuous-branch null) — NOT a mirror of the integer null; model class now decided.** The S295 build attempt confirmed at source that the integer-branch parametric null (`duplicateDetection.js:58-126`) is a **discrete count model** — Poisson/NB PMFs defined only on integers, values rounded via `Math.round` before the fit, collision probability summed over integer support at unit step — and does NOT extend to continuous 2-decimal data (applied bare it overestimates the collision rate ~100× and re-collapses p toward 1 by a different mechanism). So the continuous null is a **new model, not a branch-condition change.** The null-constraints read then bounded the choice: downstream tolerates a granular simulated fraction (no closed-form requirement), the integer ceiling is accuracy-bound not compute-bound, and — decisively — the baseline must NOT be estimated from the observed value-repetition frequencies (that coupling is the circularity, in both the malicious and the benign-quantisation direction). That rules out the empirical HHI and rules out a bootstrap over the observed multiset. **Decision (S295): density-integration at recording precision** — collision probability as the integral of the squared value-density over the recording grid (`step = 10^-dominantDp`, already computed at `duplicateDetection.js:39`). A rescaled-discrete family was rejected (a tight rescaled continuous distribution is under-dispersed for Poisson/NB, so it would need a new discrete family with no offsetting benefit); a from-fitted-model simulation collapses into this same model by another route. **One open sub-decision:** the density estimator — a parametric fit (cheap, closed-form, holds across all N, but carries misspecification risk on real biological data) versus a recurrence-robust kernel estimate (no distributional assumption, but must be built so the recurrence being tested does not inflate the density at the recurring values, or it reintroduces the circularity through the back door). This sub-choice is gated on seeing the estimator behave on the CORPUS-03 shape versus a clean quantised column — a small targeted read, not a build.
...
- **Fixture (the batch-invisible constraint, now specified).** A standing four-column fixture — `recur` (continuous 2-decimal, structured 4× recurrence, the axis-2 carrier), `wide` (wide-magnitude, the Benford axis-1 carrier), `precA`/`precB` (differing precision, the Decimal-Precision axis-1 carrier), ≥100 rows — with a `recur`-alone control isolating the axis-2 null half from the dispatch half. This one fixture serves the whole §2.6 arc and must be built alongside any fix, or only a corpus run surfaces a regression.

**Remaining §2.6 gates (both Chat design passes, neither a Code prompt yet):** (a) the axis-2 density-estimator sub-choice (parametric vs recurrence-robust KDE), gated on a small estimator-behaviour read; (b) the axis-1 rejection-predicate/threshold design pass for the three per-test guards. The owed source-comment correction (`duplicateDetection.js:135`) is independent of all of this and rides the next Code dispatch.

**Priority:** Real-world-validated and credibility-bearing for the review paper's methods section (an honest disclosed-limitations pass strengthens the validity claim). The continuous-HHI counterexample (axis 2) is the most defensible single item — a documented safe-claim falsified by external data. As of S295 its **null-model class is decided (density-integration at recording precision); the fix is gated on one design sub-decision (the density estimator) and its regression fixture**, not on implementation time — so it is no longer a bare "natural first fix," it is a scoped fix two small steps from a dispatch. Bank for v1.x; the source-derivation read ran S295 and this entry now carries its verdicts. Candidate to compete with the review-paper track at scope time.

**Source:** S293 conversation (CORPUS-03 adjudication: Benford pooling probe, Exact-Duplicate null/dispatch trace, digit-cluster dedup discriminator) + the S293 Code read of the closed item-28 audit (`docs/shared/archive/TEST-INTEGRITY-AUDIT.md`, METHODOLOGY §1.1, `duplicateDetection.js:135`); `SESSION293-CHAT-SUMMARY.md`. The candidate-further-axes list is a seed pending a source-derivation read, not a settled taxonomy.
