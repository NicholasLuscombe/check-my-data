...
**Remaining §2.6 gates:**
- **The fix itself — blocked on the collision null's expected-rate design (S299 lead).** The engine change replaces the empirical-HHI collision null on continuous columns so structured recurrence fires HIGH. The null path is read and isolated (S298: `duplicateDetection.js:55/:147/:179-184`, one call site), the per-column stratified combine is confirmed clean, and per-column grid step is locked. What is NOT settled is the baseline: both density-integration forms tried (smooth integral, cell-discretized) failed the same way — over-firing on legitimate high-precision continuous data. The fix waits on a baseline that satisfies the pinned constraint (see the S298 entry: model the coincidental quantized-repeat rate directly, neither from observed repeats nor from a smooth density). Fixture `23`'s DupDet assertion still flips LOW→HIGH when the fix lands; that flip and the fixture are unaffected by the estimator reopening.
- **The collision-null expected-rate design pass (S299 lead).** A Chat statistical-reasoning pass to name a baseline satisfying the constraint, then a read testing it against all 25 fixtures (must flip 23, must hold the seven benign movers LOW). This is the fix's critical path — it is not a later or independent pass.
...
**Priority:** Real-world-validated and credibility-bearing for the review paper's methods section (an honest disclosed-limitations pass strengthens the validity claim). The continuous-HHI counterexample (axis 2) is the most defensible single item — a documented safe-claim falsified by external data. The fix-verification fixture is built and landed (S297, batch 25/25 — all three carriers reproduce, collateral declared via `ACKNOWLEDGED`). The remaining work is the null-path fix, now blocked on the collision null's expected-rate design: the S298 builds established that the "density-integration at recording precision" model class is quantization-blind and reopened it (see the S298 entry), so the fix waits on a baseline satisfying the pinned constraint. Bank for v1.x. Candidate to compete with the review-paper track at scope time — and the paper's §5 disclosed-limitations subsection now discloses axis 2 as design-blocked, an honest limitation that strengthens the validity claim rather than weakening it.
