...
**Priority:** Real-world-validated and credibility-bearing for the review paper's methods section (an honest disclosed-limitations pass strengthens the validity claim). The continuous-HHI counterexample (axis 2) is the most defensible single item — a documented safe-claim falsified by external data — and is the natural first fix. Bank for v1.x; the source-derivation read opens it as its own lead, candidate to compete with the review-paper track at scope time.

**Source:** S293 conversation (CORPUS-03 adjudication: Benford pooling probe, Exact-Duplicate null/dispatch trace, digit-cluster dedup discriminator) + the S293 Code read of the closed item-28 audit (`docs/shared/archive/TEST-INTEGRITY-AUDIT.md`, METHODOLOGY §1.1, `duplicateDetection.js:134`); `SESSION293-CHAT-SUMMARY.md`. The candidate-further-axes list is a seed pending a source-derivation read, not a settled taxonomy.
