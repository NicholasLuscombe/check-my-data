...
| C25 | persistent luminescence, *Nature Photonics* 2024 | 10.1038/s41566-024-01396-0 | science-detective | **run** | no | **measured S373** — 14 sheets; largest `Fig. 3g` 1,362 × 20, **column-groups** into 5 (4/4/4). **The corpus's only column-grouped file.** 8 sheets column-group above 100 valid rows, to `Fig. 2d` at 3,600 rows over 3 groups; 8 exceed three groups, `Fig. 2f` at 7. **Every group is a plotting pair carrying an axis column — P93 applies to all of them.** | The phosphorescense data for Figure 2b has a big block of duplicated intensity values. Below I've put rows 20-74 next to rows 75-129 to show that they have the exact same values. https://pubpeer.com/publications/57EFF3B48FF332A7731D85C3ABC27D?ref=sciencedetective.org | §2.4 HIGH |
...
CORPUS-01, CORPUS-02 and CORPUS-03 keep their original Shape cells. Those came from the Tier-1 runs at S292–S294 and were never inference, and **the census did not load them** — see §0.3's route-check note.
...
**Two rows are reported as reproducing and cannot have been.** The census enumerated 27 files — C07–C25 plus eight update pairs — and its sheet arithmetic closes exactly on that set at 130 sheets, 119 loaded and 11 empty. **CORPUS-01 and CORPUS-03 are outside it.** Their rows here are unchanged and unverified by S373; treat the S327 figures as the standing record until someone loads those two files by name.
...
**Not met — conditions that are genuinely related.** This is the requirement P83's remaining half turns on, and nothing measured at S373 supplies it. Eight C25 sheets exceed three groups, `Fig. 2f` at seven, which is the structural precondition and not the answer. **Every C25 group is a plotting pair** — `Time (s)` with `Intensity (a.u.)`, or `Wavelength (nm)` with `Intensity (a.u.)` — so half of each group's columns is an axis, which is P93's defect on the file P93 was found on. A correlation measurement taken there inherits it.
