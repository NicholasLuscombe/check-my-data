...
### CORPUS-01 — Parkinson's / Cell 2016 (documented defect CAUGHT — the miss converted, S329)
...
- **False-positive note:** the one HIGH flag on this dataset is Missing Data Pattern, adjudicated **B2** — it localises to a tight contiguous block of one column in one group, which is honest group-specific attrition, not a diffuse anomaly. The 17 zero-weight Sequential Duplication sequences all sit in `Hindlimb score`, an integer 0/1 ordinal where short runs recur by construction; the null prices them at pAdj = 1 and they contribute nothing to the verdict — the pricing correctly keeps innocent runs out of the evidence.
