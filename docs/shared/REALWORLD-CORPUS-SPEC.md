...
> **S317 — READ §0.3 BEFORE RUNNING ANY FILE.** Six of the twelve row-grouping corpus files partition into units the row-grouped tests were not designed for. **Their row-grouped verdicts are uninterpretable until the grouping contract is settled.** Four of the six are the ecology cluster. **The cluster is blocked.** See §0.3 and V1X §2.10.
...
|---|---|---|---|---|---|---|---|---|
| CORPUS-01 | Sampson, *Cell* 2016 | 10.1016/j.cell.2016.11.018 | science-detective | **run** | — | wide, group-indexed | sequential block duplication across groups → Class C at Tier-1, now default-path catch post-§2.4 | §2 |
...
| CORPUS-03 | Bierbach, *Nat Comms* 2017 | 10.1038/ncomms15361 | science-detective | **run** | — | ID-indexed, 4 obs/fish | every SL value ×4 (join scramble) → Class A, under-called | §2 |
...
**The six in bold are blocked. Do not read their row-grouped results as findings.**
...
**Run protocol (settled at C25):** deposits are often multi-sheet workbooks — identify the PubPeer-flagged sheet/columns first, run that sheet as deposited (honest false-positive surface), and isolate to analysis-relevant columns only to adjudicate a confound. The raw-vs-isolated delta is itself a result (a B2 with a named mechanism). Read the source file to settle ambiguous flags; do not adjudicate from the console alone.
...
## Open items
- **ROW-GROUPING — BLOCKS SIX FILES AND THE WHOLE ECOLOGY CLUSTER (S317).** The engine groups rows by the Cartesian product of every column role inference tags `condition`. C12 → 132 groups; C16 → 60 singletons, silently dropped and not assessed; C22 → 44; C20 → 37; C08 → 35; C09 → 20. **The permutation null's exchangeability assumption is false about these units.** See §0.3 and V1X §2.10. **The S318 lead, and the first move is Chat authoring the contract in METHODOLOGY, not a dispatch.** Nothing row-grouped from those six files is interpretable until it is settled.
- **C14 — two things at once, never properly run.** An open adjudication (rows 260↔261 byte-identical across all twelve non-null columns — same tree, same activity year; defect or repeated-measures convention?) *and* the file that broke §2.9's prefilter (`CROWNCLASS`: five distinct values over 9,398 rows → 16.9 million agreement pairs). **C14 does not row-group** — no condition columns — so it is not blocked by the item above.
