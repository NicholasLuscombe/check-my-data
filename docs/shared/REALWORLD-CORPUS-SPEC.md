...
> **S317 — READ §0.3 BEFORE RUNNING ANY FILE.** Seven of the row-grouping corpus files (six in the S317 census + C14, corrected S322) partition into units the row-grouped tests were not designed for. Four are the ecology cluster. **The grouping contract is settled, cross-validated, and enforced** — the trigger routes these to N/A pending confirmation and the confirm card is built (S320–S322, twelve fixes). **The stack promoted at S323 (`07887a5`), verified S367 — the promote gate is discharged and this note asserted otherwise for forty-four sessions. The cluster's status is UNDETERMINED, not blocked-on-promote:** one display reconciliation against `METHODOLOGY.md` §Applicability is named in §0.3 and has not been read. **Read it before running the cluster.** See §0.3 and V1X §2.10.
...
**Lifecycle states:** `run` (battery run + adjudicated — Tier-1 detail in §2, road-test detail in §0.4) · `parked` (identified, data-access gated, §3) · `triage` (candidate, per-case fill outstanding) · `runnable` (availability confirmed and **shape measured at S373 — see the Shape column**, not yet run) · `paper-only` (no deposited data — read-only, can't road-test) · `dropped` (duplicate/out-of-scope, reason recorded).
...
|---|---|---|---|---|---|---|---|---|
| CORPUS-01 | Sampson, *Cell* 2016 | 10.1016/j.cell.2016.11.018 | science-detective | **run** | — | **measured S373** — 1 sheet; `Sheet1` 105 × 4, condition cols `Treatment` and `Genotype`, row-groups into 10 (6/10.5/15). | sequential block duplication across groups → Class C at Tier-1, **converted to a Class A catch S329** — §2.4 Sequential Duplication HIGH (1.845e-6) on the two adhesive runs + pole pair, deposit-verified | §2 |
| CORPUS-02 | Mohammadi, *PLOS Genetics* 2022 | 10.1371/journal.pgen.1010323 | science-detective | **run** | — | **measured S373** — 2 sheets. **The manifest runs `IC50`:** 197 × 8, one label col, does not group, five header blocks. The larger `ATPase Activity` (100 × 19) was not run **and is not fit to be as it stands** — `Mean` and `Standard Deviation` read as label columns and sample-name strings like `1A; 1B` read as data columns. See Open items on corpus extensions. | exact dup + near-dup terminal-digit + admitted variance manip → Class A+B1+C. **Retracted.** | §2 |
| CORPUS-03 | Bierbach, *Nat Comms* 2017 | 10.1038/ncomms15361 | science-detective | **run** | — | **measured S373 under default inference, which is NOT the shape Tier-1 ran** — 1 sheet, `Clonal molly behavioral individ` 373 × 3, condition col `Trt`, label col `Obs`, row-groups into 3 (121/124/128). **Inference classifies `Fish.ID` as data**, so the censused matrix carries three columns; `corpus-manifest.json` pins `Fish.ID` to identifier and the Tier-1 run analysed two. §2's Benford pooling account describes the two-column matrix and is correct about the run. **The hint is load-bearing, and it lives in the runner's manifest rather than in the product** (P160). | every SL value ×4 (join scramble) → Class A. Exact Duplicate under-calls (p=1), but **Sequential Duplication catches it HIGH (1.47e-35) on SL S329**; Benford HIGH is a B2 pooling FP | §2 |
...
| C21 | precipitation experiment | 162 | 9 | 18 | 18 | treatment |
...
| C10, C18, C19, C23, C25, CORPUS-02 | — | 0–3600 | 0 | — | — | no condition columns — not row-grouped. **S327 sheet corrections:** C10's flagged tab is `P. megatetrium Experiment1` — 848 raw rows, 633 × 28 through the pipeline. C25's flagged sheet is `Fig. 2b`. **Corrected S373: 401 parsed rows × 8 data columns in 4 column groups, with 401 valid rows — not 401 × 12 resolving to 400.** The raw sheet is 2,003 × 11 and **1,600 rows are trimmed as sparse**, because the phosphorescence group runs to 800 nm at fine steps while the other three stop early. `Fig. 2c` loses 39,602 of 43,202 the same way. **Nothing announces either trim, and the pattern is corpus-wide rather than a C25 defect — P158.** C10's `B. cereus Experiment1` goes 16,522 raw rows to 400; C14's `Metadata` 16,396 to three; C11's `snRNA-seq_Fig 7` 9,279 to three. **A reader sees the surviving count and nothing else.** Neither crosses the 5,000-row sequence-scan guard. **C25 is also the corpus's largest column-grouped file** — twelve of fifteen such sheets, C15 holding the other three — which this catch-all row's "not row-grouped" heading is true about and silent on. |
| **C15** | Data | 60 | **14** | **3.5** | **3** | **Imports as of `d3f50e9`** (S326 import-width trim). Resolves to 60 × 18 with five condition columns. The earlier reading — "does not import; `preprocessRaw` computes the sparse-row threshold from the used range, which runs to column XFD → threshold 1639 against ~26 real values per row; row-grouping unknown and untestable" — described the pre-trim engine and is retired. Import confirmed independently S327. **Group count and median measured S373: 14 groups, median 3.5, minimum 3, on five condition columns.** The `5` this row previously showed in the *Groups* column was the condition-column count in the wrong cell — the row asserted a group count and denied having one, in the same cell. |

*S322 correction.* This is the S317 census, "largest data sheet per file." The S322 firing-rate sweep re-ran the same `computeTrigger` through the real inference pipeline across all 22 deposits, reproduced every measured row here bit-identically, and corrected the catch-all on one file: **C14 fires** — its `Data` sheet (9,426 parsed rows, **9,398 valid after completeness filtering — S373; C14 is the only file where the two diverge materially**) tags `Species` + `DamageSev` → 236 groups, min size 1 → Arm 2. C14 is the 7th fire, wrongly bucketed above as "no condition columns"; treat it as blocked with the six. **C24** is row-grouped but clean (tags `Month` → 4 healthy groups). Full 7-of-22 result and the bimodal domain split (47% of row-groupable ecology files, 0% of assay/instrument/expression) in V1X §2.10.
...
**Route check (S373).** The S373 shape census re-ran this table's measurements through the same inference path. **Ten of the twelve rows reproduce exactly** — C12, C16, C22, C20, C08, C09, C21, C07, C13 and C17 — plus C14's 236 groups and C10's 633 × 28. Five corrections are folded in above.

**All twelve reproduce, including the two CORPUS rows.** CORPUS-01's `Sheet1` measures 105 rows, 10 groups, min 6 median 10.5; CORPUS-03's `Clonal molly behavioral individ` measures 373 rows, 3 groups, min 121 median 124. Both exact against the figures below.

**Corrected S373, same session.** An earlier draft of this note claimed those two rows "cannot have been" measured, reasoning that the census enumerated 27 files over 130 sheets and the CORPUS deposits sit outside that set. **The premise was right and the conclusion was wrong.** The probe carries a second array, `ADDENDUM`, holding CORPUS-01, -02 and -03, run through the identical route and reported under a separate key — 119 primary sheets plus 4 addendum, 123 in all. The 27-file arithmetic closed over a real set and was silent that a second set existed. **A reconciliation that closes proves the set it closes over is consistent. It says nothing about whether that set is the whole set.**

**This table reads one sheet per file, and that is where its coverage stops (P159).** Thirteen sheets corpus-wide row-group into unusable units; this table records four. **C11 appears nowhere in it** — not in the table, not in the catch-all — and carries the corpus's worst explosion: `Cell cycle scores_Fig 2b` tags **seventeen** condition columns into 63 unusable groups, against C12's seven into 132, with four more C11 sheets unusable beside it. C12's `Field survey-Herbiory` reaches 324 groups and is also unlisted. The cause is this table's own method rather than any measurement in it: the largest data sheet per file is not where the worst grouping lives, and C11's largest sheet groups neither way. **C11 is lifecycle `run`.**

**Three "largest data sheet per file" labels disagree with the census's cell-count ranking** — C13 (`Soil CO2` here, `Leaves to Soil` by cell count), C17 (`Neural` / `Cognition`) and C20 (`Microcosm soil B` / `Microcosm soil A`). **The numbers on the sheets this table names all reproduce**, so this is a question about which sheet is largest, not a conflict between two measurements of one sheet.
...
## 7. Round 2 — what the corpus still cannot exercise (S373)

**The shape gap is one requirement, not three.** P141 recorded three shapes the fixture corpus cannot supply — wide, tall and two-column deposits — and treated them as a reason to collect. The S373 census measured the deposits already on disk and **two of the three are met**, on files downloaded at S305 and never run.

**Met — a two-column file, making `nPairs < 2` reachable.** Seven sheets present exactly two data columns. The clean exhibits are `C13-updated.xlsx`'s `Plant 13C` (323 valid rows), `Plant 90-360d` (162) and `Soil90-360d` (72), where `d 13C/12C` and `Amt%` are genuinely the only measurements and the remaining columns take condition and label roles. Nothing was collapsed to get there; the sheets were read at source. C11's two large sheets reach two columns by the opposite route — an eleven-column sheet holding two panels side by side, stripped by the sparse-column pass — and are not the exhibit.

**Met — a tall column-grouped file.** Eight C25 sheets column-group above 100 valid rows, to `Fig. 2d` at 3,600 rows over 3 groups, against the four column-grouped fixtures at 35 and 60.

**Met past what was anticipated — width.** C16's `Sheet1` is 99 data columns, giving `C(99,2) = 4,851` column pairs against the widest fixture's 171. That is P136's worst case sitting in `corpus-data/`.

**Not met — but present and unmeasurable rather than absent.** This is the requirement P83's remaining half turns on. Eight C25 sheets exceed three groups, `Fig. 2f` at seven, which is the structural precondition and not the answer.

**The relatedness is probably there and this file cannot measure it.** `Fig. 3g`'s five groups are named `cycle 1` through `cycle 5` — one measurement repeated five times on one sample, which is relatedness by construction rather than by accident. **But half of every group's columns is a shared time axis**, so any correlation computed across those groups cannot separate related physics from a time grid recorded five times. That is P93's defect, on the file P93 was found on.

**Corrected S373:** an earlier draft of this section said nothing measured supplies the requirement. What the census supports is narrower — the shape is present in C25 and confounded at the point of measurement.

**So round 2 is scoped to one shape, not three:** a deposit whose conditions are related by the experiment rather than by a shared plotting axis, column-grouped, with more than three groups.

**Two exercisable-not-free caveats, both to be stated before any run is scoped.**

- **C25 carries P93 on every column group.** Any P83 measurement on it is confounded at the point of measurement.
- **C16 sits behind the grouping-confirmation trigger.** It row-groups into sixty singletons, so reaching its 99 columns requires a user decision, not a file. That makes P136's worst case reachable rather than available.

**Say which direction a run on either file can settle before scoping it.** Both can exonerate; neither can convict on its own.

**What round 2 does not need.** Nothing here argues for collecting wide, tall or two-column deposits. Anyone reaching for that requirement should read this section and the Shape column first.

---

## Open items
- **ROW-GROUPING — BLOCKS SIX FILES AND THE WHOLE ECOLOGY CLUSTER (S317).** The engine groups rows by the Cartesian product of every column role inference tags `condition`. C12 → 132 groups; C16 → 60 singletons, silently dropped and not assessed; C22 → 44; C20 → 37; C08 → 35; C09 → 20. **The permutation null's exchangeability assumption is false about these units.** See §0.3 and V1X §2.10. **The S318 lead, and the first move is Chat authoring the contract in METHODOLOGY, not a dispatch.** Nothing row-grouped from those six files is interpretable until it is settled. **(S322 update: contract settled and cross-validated twice; both parts built — trigger + confirm card, stance validated, twelve fixes. C14 is a 7th blocked file, S322 census correction. See §0.3 and V1X §2.10.)** **(S367 correction: promoted at S323, merge `07887a5`, verified — the promote gate is discharged. The display reconciliation against `METHODOLOGY.md` §Applicability is now the only named gate and is unread.)**
