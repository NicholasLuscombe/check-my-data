...
- **update scope:** *(fill — identical PCo1/PCo2 for rows of differing richness; note this is a derived-column contradiction, not whole-row dup — an update may "fix" the coordinate without resolving the inconsistency)*
- **adjudication target:** *(fill)*
...
## 0.3 Eyeball-findings log (per-case, adjudicated against source — S305)
...
2. **The applicability failure produces false *negatives* too.** C08's Exact Duplicate cleared a file with 25.5% within-column duplicates, because its empirical-HHI null was estimated from the contaminated distribution. C12's Duplicated Data returned High while pointing at the wrong columns. **The same defect that over-flags honest structure also suppresses and misdirects real signal.** This is the sharper half of the disclosure and it is not in the literature.
