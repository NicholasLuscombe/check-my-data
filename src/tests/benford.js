...
export function testBenford(matrix, rng) {
  ...
  if(actualSpan<1.5) return {name:"Benford's Law (First Digit)",category:"digits",flag:"N/A",naCause:NA_CAUSE.RANGE_TOO_NARROW,description:`OOM span ${actualSpan.toFixed(1)} < 1.5 — Benford's Law requires ≥1.5 orders of magnitude for meaningful application.`};
