...
export function MiniCard_ColumnGoF({ result, importConfig, rowMap }) {
  ...
  const lookForText = nHigh > 0 && nLow === 0
    ? `Check whether the flagged columns carry the key results. Inspect each column's histogram — the average and spread will look normal, so focus on the tails and any extra peaks or clustering, where the signal sits. Cross-reference Multiple peaks, which catches the bimodal case directly.`
    : nLow > 0 && nHigh === 0
    ? `Check whether the flagged columns carry the key results. Compare them against the raw data files: values matching their predicted shape this closely are hard to obtain by measurement. Cross-reference Value diversity — generated values that fit too cleanly often also show unnaturally even spacing.`
