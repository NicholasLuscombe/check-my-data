  ...
  return (
    <MiniCardLayout result={result}
      footer={result.flag !== "LOW" && result.flag !== "N/A"
        ? (isPlat ? "Noise shape too flat and wide" : "Noise shape too peaked and narrow")
        : "Noise shape as expected"}
      lookFor="Inspect the raw data files and examine the replicate differences: values spread evenly across a range, rather than clustering near zero, point to noise drawn from a flat distribution. Check whether the pattern holds across the whole dataset or concentrates in particular conditions, using the per-condition breakdown. Cross-reference Within-row uniformity: rows whose replicates are too alike, alongside this too-flat shape, locate where the uniform noise was added — the dataset-wide shape here, the specific rows there."
      implications={'A too-flat spread of replicate differences points to noise added from a uniform range — picking each value evenly between limits — rather than the bell-shaped noise real measurement produces. It is a common signature of replicates generated by adding "random" noise that is actually uniform, not Gaussian.'}>
