...
export function MiniCard_RankCorrelation({ result, importConfig, rowMap }) {
  ...
  const rhoVals = details.map(d => parseFloat(d.spearmanR)).filter(v => !isNaN(v));
  ...
  // ── Headline ──
  let headline;
  if (result.insufficientPairs) {
    headline = `Only ${nPairs} condition pair${nPairs > 1 ? "s" : ""} available — insufficient for statistical inference (≥3 needed).`;
  } else if (result.flag === "LOW") {
    headline = "Rank ordering varies between conditions as expected — no pair is suspiciously similar.";
  } else if (nSusp >= 2) {
    headline = `${nSusp} condition pairs have near-identical rank ordering (ρ ≥ ${maxRho.toFixed(2)}) — the conditions may not be genuinely independent.`;
  } else {
    headline = "One condition pair has suspiciously similar rank ordering — features rank nearly identically across both conditions.";
  }

  const desc = "Checks whether different conditions produce unusually similar patterns. The rank order of rows tends to vary between conditions.";
