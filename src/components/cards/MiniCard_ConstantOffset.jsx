...
export function MiniCard_ConstantOffset({ result, importConfig, rowMap }) {
  ...
  const nBlocks = parseInt(result.consecutiveEqualDiffs) || 0;
  ...
  if (result.flag === "LOW" || result.flag === "N/A") {
    headline = nBlocks === 0
      ? "No consecutive rows share the same inter-replicate difference."
      : `${nBlocks} adjacent equal-difference pair${nBlocks !== 1 ? "s" : ""} found — within chance expectation (${expBlocks.toFixed(0)} expected).`;
  } else if (result.severityClass === "high") {
    headline = `${nBlocks} adjacent row pairs share identical inter-replicate differences — ${(nBlocks / expBlocks).toFixed(1)}× more than expected by chance.`;
  } else {
    headline = `${nBlocks} adjacent row pair${nBlocks !== 1 ? "s" : ""} share identical inter-replicate differences — more than the ${expBlocks.toFixed(1)} expected by chance.`;
  }
  ...
  return (
    <MiniCardLayout result={result} headline={headline}
      ...
      footer={`${result.offsetType === 'multiplicative' ? '(multiplicative) ' : ''}${blockEntries.length} block${blockEntries.length !== 1 ? "s" : ""} detected · ${expBlocks.toFixed(1)} expected by chance · permutation p = ${fmtP(result.primaryP)}${result.nPerm ? ` (${result.nPerm} perms)` : ""}`}
