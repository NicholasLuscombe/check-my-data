/* ── ColumnStatBar — per-column statistic bar with reference line ──
   ...
   ratio = 1, AD² null median = 1, dip-magnitude gate).
   ...
   v1.x distribution-histogram with fitted-reference overlay (parked
   #7) is out of scope here. */

import { C, CP, CS, CF, FF } from "../../constants/tokens.js";
...
/**
 * @param {object} props
 * @param {Array<{colLabel:string,value:number,flagged:boolean}>} props.items
 ...
 * @param {string} props.refLabel - reference line caption
 * @param {string} props.valueAxisLabel - y-axis label
 */
export function ColumnStatBar({ items, cardFlag, refValue, refLabel, valueAxisLabel }) {
  if (!items?.length) return null;

  const flaggedColor = SEV_VERDICT[FLAG_RANK[cardFlag] ?? 0].color;

  const W = CP.W_MD, H = 140, PL = 44, PR = 12, PT = 18, PB = 34;
  const CW = W - PL - PR;
  const CH = H - PT - PB;
  const vals = items.map(d => Number(d.value) || 0);
  ...
  const bw = Math.max(2, Math.floor(CW / items.length) - 4);
      ...
      <PlotSVG W={W} H={H} overflow>
        ...
        {Number.isFinite(refValue) && refValue >= 0 && refValue <= mx && (
          <g>
            ...
            {refLabel && (
              <text x={W - PR} y={yscale(refValue) - 4} fontSize={CF.SMALL} fill={C.TEXT_3}
                textAnchor="end" fontFamily={FF.UI}>{refLabel}</text>
            )}
          </g>
        )}
        ...
        {items.map((d, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={x + bw / 2} y={H - PB + 12} fontSize={CF.LABEL} fill={C.TEXT_3}
                textAnchor="middle" fontFamily={FF.MONO}>{d.colLabel}</text>
            </g>
          );
        })}
