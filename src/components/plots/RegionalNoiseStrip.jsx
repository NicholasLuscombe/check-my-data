...
export function RegionalNoiseStrip({ details, nRows, colNames, toFileRow }) {
  ...
  const W=CP.W, ROW_H=16, GAP=4, PL=72, PR=16, PT=8, PB=38;
  const CW=W-PL-PR;
  ...
  return (
    <PlotSVG W={W} H={H}>
      ...
      <text x={8} y={PT + cols.length*(ROW_H+GAP)/2} textAnchor="middle"
        fontSize={CF.AXIS} fill={C.TEXT_3} fontFamily={FF.UI} transform={`rotate(-90,8,${PT + cols.length*(ROW_H+GAP)/2})`}>Column</text>
      {cols.map((col, ci) => {
        const y = PT + ci*(ROW_H+GAP);
        const wins = windowsByCol[col];
        const colNum = parseInt(col);
        const label = (colNames && colNames[colNum]) || ("Col " + col);
        return (
          <g key={ci}>
