/* ── ForensicsBody — Forensics document branch §2 + §3 chrome ──
   Owns:
     - §2 WHAT WAS FOUND  — section header + chip lanes + sticky panel.
                            Chip lanes (StickySurface) render in normal
                            flow; FindingDetailPanel sticky-pins to
                            viewport top for the active region. Clean
                            (no findings) renders an "all checks passed"
                            line in place of the chip lanes.
   ...
   Click model:
     - chip click       → pulse(chip:N, region:N, card:<each>) + scroll
                          card + activate region (S163 Phase 3b: chip
                          click sets activeRegionNumber, opening the
                          FindingDetailPanel)
     ...
     - region [N] badge → pulse(region:N, chip:N) + onActivateRegion
                          (sets activeRegionNumber → panel pins for
                          that region; badge lives on the panel's
                          internal MinimapStripVertical now)

   S163 lifecycle: `activeRegionNumber` is the §2 state and drives the
   FindingDetailPanel mount + the per-chip `showRegionNumber` derivation
   in StickySurface. Initial value null → panel not auto-open on load.

   S163 Phase 3d (A1.D3 close): position: sticky chrome migrated from
   StickySurface (now normal flow) to FindingDetailPanel. The
   horizontal MinimapStrip mount retired in favour of the panel-
   internal MinimapStripVertical. DeepLookModal retired entirely. */
...
// Selector that scrollToCard reads to offset chip-click landings past
// the sticky panel. The DOM marker lives on FindingDetailPanel.
const FINDING_PANEL_SELECTOR = "[data-finding-detail-panel]";
...
export function ForensicsBody({
  ...
  // / quotes. The arithmetic offsets the landing position past the
  // pinned FindingDetailPanel — when the panel is mounted, its
  // offsetHeight is the gap to leave; when it isn't, the card lands
  // BREATHING_MARGIN below viewport top.
  //
  // S133f-fix2 (the pre-3d arithmetic) is preserved in shape; only the
  // selector target changed from StickySurface to FindingDetailPanel as
  // part of the Phase 3d sticky migration.
  const scrollToCard = useCallback((testId) => {
    ...
    if (!el) return;
    const panelEl = document.querySelector(FINDING_PANEL_SELECTOR);
    const panelHeight = panelEl ? panelEl.offsetHeight : 0;
    const elTop = el.getBoundingClientRect().top;
    const target = window.scrollY + elTop - panelHeight - SCROLL_BREATHING_MARGIN;
  ...
  return (
    <>
      {/* §2 WHAT WAS FOUND — three DOM elements that merge visually
          into one §2 surface. Section header in its own flat-bottom
          wrapper, StickySurface (chip lanes, normal flow) as a flat-top
          continuation, and FindingDetailPanel below them as the §2
          sticky element when a region is active. All three are
          Fragment siblings of §3-§5 so the panel's `position: sticky`
          pin range spans the whole report (sticky is bound by its
          PARENT's bottom edge; the parent is the ForensicsBody
          Fragment / ReportView outer wrapper).

          Clean state (no findings): no chip lanes, no panel — §2
