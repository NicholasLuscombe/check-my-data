...
export const TEST_METHODS = {
  ...
  // --- Unusual Digits ---
  'Terminal Digit Uniformity': 'In real measurements the last recorded digit is essentially random — each of 0–9 should appear about equally often. This test compares the observed spread of final digits against that flat expectation; a strong lean toward particular end-digits (round numbers, a favoured few) is a signature of numbers written by hand rather than measured.',
  ...
  'Selective Noise Partitioning': 'Checks whether all replicate columns have similar variability. A significant result means the noise levels are unequal (Bartlett\'s test), but with few columns it may not be possible to pinpoint which one differs. The ratio is how many times more (or less) variable the flagged column is than the others — a column at 7× carries seven times the noise of its neighbours, which is hard to produce by a uniform measurement process and easy to produce by editing one column in isolation.',
  ...
  'Autocorrelation': 'Checks whether the inter-replicate noise is correlated between adjacent rows. In independent measurements, knowing the noise at one row tells you nothing about the next \u2014 significant correlation means the noise follows a pattern rather than varying freely (lag-1 autocorrelation, pooled t-test). This is distinct from Shared noisy rows and Noise sign-pattern. Where those ask which rows are noisy or which replicate leads, this test asks only about the magnitude of the noise from one row to the next — whether a noisy row tends to be followed by another noisy row, a smoothness that hand-edited data often shows and independent measurement noise does not.',
  ...
  'Runs Test': 'For each replicate pair, tracks which replicate has the larger value at each row and checks how often the lead switches. Too few switches means one replicate stays consistently above the other \u2014 either across the full column or within localised stretches (Wald\u2013Wolfowitz runs test). This is distinct from Shared noisy rows and Noise correlation, which both concern how large the noise is. This test ignores magnitude entirely and looks only at the sign sequence — which replicate is higher at each row — testing whether the lead switches as often as random noise would. Too few switches means the signs run in streaks, which is hard to fake by eye.',
  ...
  'Cross-Condition Rank Correlation': 'Checks whether different experimental conditions produce unusually similar row-level patterns. In typical experiments, the rank order of rows varies between conditions \u2014 consistently similar rankings across multiple condition pairs suggests shared underlying structure (Spearman rank correlation). This is distinct from Baseline Balance and Overall condition similarity. It asks whether two conditions produce the same row-by-row ordering — whether the rows that rank high in one condition rank high in the other, an agreement too strong to arise if the conditions were measured independently.',
  ...
  'Baseline Balance': 'When subjects are randomly assigned to groups, some measurements \u2014 like age or weight \u2014 will naturally differ a little between groups. If every measurement matches almost perfectly across groups, the assignment may not be genuinely random. This test checks whether the between-group differences are too consistently small (Carlisle\'s method). Each feature is compared across the conditions with a one-way test (ANOVA), and the spread of those per-feature p-values is checked: genuine allocation scatters them evenly, while over-balancing crowds them toward 1. This is distinct from Cross-Condition Rank Correlation and Overall condition similarity, which look for conditions that are too alike. Baseline Balance looks at the opposite failure: pre-treatment group differences that are suspiciously too small — closer to zero than real randomisation would deliver, the signature of groups built to match rather than randomly assigned.',

  'Cross-Condition Consistency': 'Compares each condition pair on a set of robust distribution properties (trimmed span, dispersion, CDF shape) and flags pairs that are suspiciously close to each other — condition pairs that match more than random re-assignment of pooled values would produce. Real experimental conditions legitimately differ on location and scale, so the forensic signal for these properties lives in the "too similar" tail. Two-sided permutation null at the cell level, BH-FDR across all property \u00d7 pair units; only similar-direction units contribute to the flag. This is distinct from Cross-Condition Rank Correlation, which compares row ordering, and from Baseline Balance, which compares group means. This test compares the overall shape of each condition\'s distribution — spread, dispersion, and tail behaviour together — and flags condition pairs that match across all of these at once, a uniformity that is hard to produce except by copying. Real treatments change a condition\'s level and spread, so larger differences between conditions are expected and reported only for context; this test flags the opposite — conditions that are more alike than honest variation would produce. Because significance is judged against a permutation distribution of limited size, the strongest tier this test can reach is Moderate; the High threshold is not attainable at this number of permutations (see METHODOLOGY §1.9).',
};
