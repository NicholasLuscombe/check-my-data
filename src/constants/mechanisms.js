...
export const TEST_METHODS = {
  ...
  'Autocorrelation': 'Takes the difference between each replicate pair and checks whether that difference is correlated from one row to the next (lag-1 autocorrelation). In independent measurements, one row\'s difference tells you nothing about the next. Each pair is tested on its own and the results are corrected for the number of pairs, so a pattern in one or two pairs is not diluted by the rest. It does not say where in the sequence the correlation sits — a pattern confined to one stretch of rows is the windowed test\'s job. It reads the row order as given; where that order is arbitrary, such as a gene list, row-to-row correlation has no meaning and a result here should not be read as evidence.',

  'Windowed Autocorrelation': 'Slides a 15-row window across the data and checks whether the difference between replicates is correlated from one row to the next within any window — a pattern confined to one stretch is detected here even when the dataset as a whole clears (windowed lag-1 autocorrelation, permutation null within each replicate pair). Unlike most sequence tests, it runs whatever the row order, because it compares each window against shuffles of the same pair\'s own rows. Where the order is arbitrary, a result here should not be read as evidence.',
